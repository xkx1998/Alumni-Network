...
//	}
    ...
    public String publishClassInfo(HttpServletRequest request,Model model, RedirectAttributes attr, @RequestParam("infoTitle") String infoTitle,
        ...
        if (CommonUtil.isNotEmpty(content) && CommonUtil.isNotEmpty(infoTitle)) {
            ...
            if (infoImage != null) {
                // 保存消息的图片 
                information.setThumbImage(ImageUtil.saveImage(infoImage,request));
            }
        } else {
    ...
    public String uploadImage(HttpServletRequest request,RedirectAttributes attributes, Integer classId, Integer albumId,
        ...
        if (CommonUtil.isNotEmpty(classId) && CommonUtil.isNotEmpty(albumId)
            ...
            if (album != null) {
                ...
                for (int i = 0; i < images.length; i++) {
                    // 存储图片
                    String imagePath = ImageUtil.saveImage(images[i],request);
                    // 图片对象
